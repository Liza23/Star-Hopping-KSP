...
class hop_func():
  ...
  def list_stars(self, data):
    stars_catalogue = pd.DataFrame(columns = ['star','ra','dec','mag'])
    ...
    try:
      ...
      for i in range(0, len(stars_point)):
        if min_val >= self.click.distance(stars_point[i]):
          ...
          star_name = star_catalogue['star'].iloc[i]
  ...
  def hop_near_click(self, star_catalogue, hops):
    ...
    hops = {'id': star_name, 'ra': star_point.x, 'dec': star_point.y}
    return hops


'''Reading the Star Catalogue Dataset'''
#reading catalog and dropping nan values
star_cat = pd.read_csv("data_webscraping.csv")
...
def save_hops():
  hops = pd.DataFrame(columns = ['id','ra','dec'])
  for elem in store_clicks:
  	elem = elem.split(",")
  	x = float(elem[0])
  	y = float(elem[1])

  	hop_stars = hop_func(0.004, 0.05, 0.1, x, y)
  	stars_catalogue = hop_stars.list_stars(star_cat.filter(items=['id', 'ra','dec','mag']))
