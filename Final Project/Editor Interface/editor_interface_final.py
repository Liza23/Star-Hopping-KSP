...
ty = pd.read_csv('Database/tycho-1.csv', nrows=5031)
...
# sorting objects in messier_objects.csv according to objects
    cl_ms = ms[(ms["TYPE"] == 'OpC') | (
        ms["TYPE"] == 'GlC') | (ms["TYPE"] == 'Cl*')]
    pn_ms = ms[(ms["TYPE"] == 'PN')]
    ga_ms = ms[(ms["TYPE"] == 'G') | (ms["TYPE"] == 'Sy2') | (ms["TYPE"] == 'IG') | (ms["TYPE"] == 'GiG') | (
        ms["TYPE"] == 'GiP') | (ms["TYPE"] == 'SyG') | (ms["TYPE"] == 'SBG') | (ms["TYPE"] == 'BiC') | (ms["TYPE"] == 'H2G')]
    re_ms = ms[(ms["TYPE"] == 'HII') | (ms["TYPE"] == 'As*') | (ms["TYPE"] == 'LIN')
               | (ms["TYPE"] == 'mul') | (ms["TYPE"] == 'RNe') | (ms["TYPE"] == 'AGN')]
...
# scatter messier
    ...
    flux = 10**(-mag/2.5)
    p1 = ax.scatter(cl_ms['RAJ2000'], cl_ms['DEJ2000'],
                    color='darkorange', s=17, zorder=10, edgecolor="black")
    l1 = ["[RA {0:.5f} \v DEC {1:.5f} \v Constellation {2} \v Name {3}]".format(i, j, k, l, '.2f', '.2f', '%s', '%s') for i, j, k, l in zip(
        cl_ms['RAJ2000'], cl_ms['DEJ2000'], cl_ms['Constellation'], cl_ms["MAIN_ID"])]
    t1 = plugins.PointLabelTooltip(p1, l1)
    plugins.connect(fig, t1)

    mag = pn_ms["V"].fillna(1)
    flux = 10**(-mag/2.5)
    p2 = ax.scatter(pn_ms['RAJ2000'], pn_ms['DEJ2000'],
                    color='white', s=80*flux, zorder=0)
    l2 = ["[RA {0:.5f} \v DEC {1:.5f} \v Constellation {2} \v Name {3}]".format(i, j, k, l, '.2f', '.2f', '%s', '%s') for i, j, k, l in zip(
        pn_ms['RAJ2000'], pn_ms['DEJ2000'], pn_ms['Constellation'], pn_ms["MAIN_ID"])]
    t2 = plugins.PointLabelTooltip(p2, l2)
    plugins.connect(fig, t2)

    mag = ga_ms["V"].fillna(1)
    flux = 10**(-mag/2.5)
    p3 = ax.scatter(ga_ms['RAJ2000'], ga_ms['DEJ2000'],
                    color='red', s=17, zorder=20, alpha=1, edgecolor="black")
    l3 = ["[RA {0:.5f} \v DEC {1:.5f} \v Constellation {2} \v Name {3}]".format(i, j, k, l, '.2f', '.2f', '%s', "%s") for i, j, k, l in zip(
        ga_ms['RAJ2000'], ga_ms['DEJ2000'], ga_ms['Constellation'], ga_ms["MAIN_ID"])]
    ...
    l4 = ["[RA {0:.5f} \v DEC {1:.5f} \v Constellation {2} \v Name {3}]".format(i, j, k, l, '.2f', '.2f', '%s', '%s') for i, j, k, l in zip(
        re_ms['RAJ2000'], re_ms['DEJ2000'], re_ms['Constellation'], re_ms["MAIN_ID"])]
...
# scatter ngc
    ...
    flux = 10**(-mag/2.5)
    p5 = ax.scatter(mag_ng['RAJ2000'], mag_ng['DEJ2000'],
                    c='white', alpha=0.8, s=80*flux)
    l5 = ["[RA {0:.5f} \v DEC {1:.5f} \v Constellation {2} \v Name {3}]".format(i, j, k, l, '.2f', '.2f', '%s', '%s') for i, j, k, l in zip(
        mag_ng['RAJ2000'], mag_ng['DEJ2000'], mag_ng['Constellation'], mag_ng['Name'])]
...
# scatter tycho
    ...
    l6 = ["[RA {0:.5f} \v DEC {1:.5f} \v Constellation {2} \v Name {3}]".format(i, j, k, l, '.2f', '.2f', '%s', '%s') for i, j, k, l in zip(
        mag_ty['RAJ2000'], mag_ty['DEJ2000'], mag_ty['Constellation'], mag_ty['Bayer'])]
...
# constellation borders
    ...
    plt.legend(handles=[cluster, planetary_neb2, galaxy, rest],
               labelspacing=5, ncol=4, borderpad=1, loc='lower center')
