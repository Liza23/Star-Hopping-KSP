...
data_cb = pd.read_csv('Database/constellation_borders.csv', encoding='latin1')
...
# messier objects
data_m = pd.read_csv("Database/messier_objects.csv", encoding='latin1')
...
# star data
file = pd.read_csv('Database/tycho-1.csv')
...
table = pd.read_csv('Database/name_resolver.csv')
...
def find_hop_name(ra, dec):
    x = str()
    full_hops_list = pd.read_csv("Hops/full_hops_list.csv")
    hop = full_hops_list[(full_hops_list['ra'] == ra) &
                         (full_hops_list['dec'] == dec)]
    x = str(hop['id'])
    return int(x[0:2])+1

find_hop_name(294.99879,-30.96475)
...
def get_input_values():
    ...
    hops = pd.read_csv(
        "Hops/M"+str(find_hop_name(float(ra_in[0]), float(dec_in[0])))+".csv")
